<script lang="ts">
	...
	import { dragScroll, type DotA11y, type OnChangeEvent, type OnInitEvent, type ResponsiveProperty, type Dot } from "./carousel.js";
	...
	export let scrollTo: (slide: number) => void;
	export let navigate: (slide: number) => void;
	...
	const scrollProgress = (e: PointerEvent) => {
		...
		if (track instanceof HTMLElement) {
			const pointerPositionOnTrackInPercent = (e.clientX - track.getBoundingClientRect().left) / track.clientWidth;
...
{#if slides.length > 0}
	<div aria-roledescription="carousel" {id} class={containerClass} data-carousel-container>
		<div
			...
			style:--padding-xs={`${(axis.xs || axis.default) === "x" ? "0 " : ""}${gap.xs ?? gap.default ?? 20}px ${(axis.xs || axis.default) === "x" ? "" : "0"}`}
			style:--padding-sm={`${(axis.sm || axis.default) === "x" ? "0 " : ""}${gap.sm ?? gap.default ?? 20}px ${(axis.sm || axis.default) === "x" ? "" : "0"}`}
			style:--padding-md={`${(axis.md || axis.default) === "x" ? "0 " : ""}${gap.md ?? gap.default ?? 20}px ${(axis.md || axis.default) === "x" ? "" : "0"}`}
			style:--padding-lg={`${(axis.lg || axis.default) === "x" ? "0 " : ""}${gap.lg ?? gap.default ?? 20}px ${(axis.lg || axis.default) === "x" ? "" : "0"}`}
			style:--padding-xl={`${(axis.xl || axis.default) === "x" ? "0 " : ""}${gap.xl ?? gap.default ?? 20}px ${(axis.xl || axis.default) === "x" ? "" : "0"}`}
		...
		>
			...
			{#each slides as slide, index (key ? slide[key] : index)}
				<div id={`${id}-slide-${index + 1}`} aria-label={`Slide ${index + 1} of ${slides.length} `} aria-roledescription="slide" role="tabpanel" class={slideClass} data-carousel-slide={index}>
		...
		<slot
			...
			{dots}
		/>
		<slot name="pagination" {next} {canScrollNext} {prev} {canScrollPrev} nextA11y={buttonA11y("next")} prevA11y={buttonA11y("prev")} />
...
<style>
	...
	@media (max-width: 640px) {
		...
		[data-carousel-slider] > [data-carousel-slide] {
			...
			max-width: calc(var(--layout-xs) - var(--partial-delta-xs));
		...
		:global([data-carousel-slider][data-axis-xs="y"][data-dragging="false"][data-drag-free="false"]) {
	...
	@media (min-width: 640px) {
		...
		[data-carousel-slider] > [data-carousel-slide] {
			...
			max-width: calc(var(--layout-sm) - var(--partial-delta-sm));
			padding: var(--padding-sm);
		}
		:global([data-carousel-slider][data-axis-sm="y"]) {
			flex-direction: column !important;
		}
		:global([data-carousel-slider][data-axis-sm="y"][data-dragging="false"][data-drag-free="false"]) {
			scroll-snap-type: y mandatory;
		}
	}
	@media (min-width: 768px) {
		...
		[data-carousel-slider] > [data-carousel-slide] {
			...
			max-width: calc(var(--layout-md) - var(--partial-delta-md));
			padding: var(--padding-md);
		}
		:global([data-carousel-slider][data-axis-md="y"]) {
			flex-direction: column !important;
		}
		:global([data-carousel-slider][data-axis-md="y"][data-dragging="false"][data-drag-free="false"]) {
			scroll-snap-type: y mandatory;
		}
	}
	...
	@media (min-width: 1024px) {
		...
		[data-carousel-slider] > [data-carousel-slide] {
			...
			max-width: calc(var(--layout-lg) - var(--partial-delta-lg));
			padding: var(--padding-lg);
		}
		:global([data-carousel-slider][data-axis-lg="y"]) {
			flex-direction: column !important;
		}
		:global([data-carousel-slider][data-axis-lg="y"][data-dragging="false"][data-drag-free="false"]) {
			scroll-snap-type: y mandatory;
		}
	}
	@media (min-width: 1280px) {
		...
		[data-carousel-slider] > [data-carousel-slide] {
			...
			max-width: calc(var(--layout-xl) - var(--partial-delta-xl));
			padding: var(--padding-xl);
		}
		:global([data-carousel-slider][data-axis-xl="y"]) {
			flex-direction: column !important;
		}
		:global([data-carousel-slider][data-axis-xl="y"][data-dragging="false"][data-drag-free="false"]) {
			scroll-snap-type: y mandatory;
		}
	}
	...
	:global([data-carousel-slider][data-dragging="false"][data-drag-free="false"] > [data-carousel-slide]) {
