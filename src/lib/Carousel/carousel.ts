...
export const dragScroll = (
	node: HTMLElement,
	{ id, oneAtTime = false, layout, axis = { default: "x" }, enabled = true, partialDelta, dragFree, pauseOnHover, autoHeight, autoPlay, onInit, onChange }: DragScrollParameters
) => {
	...
	const emitChange = () => {
		...
		onChange({
			...
			slidesInView: Array.from({ length: slideCount }, (_, i) => i).filter((slide) => slide >= currentSlide && slide < currentSlide + slidesPerView),
		});
	...
	const handleMouseUpAndLeave = (e: MouseEvent) => {
		...
		if (currentAxis === "x") {
			if (distanceX > 10 && distanceX > distanceY) {
				...
				const computedDistance = (distanceX * multiplier) / (dragFree ? 1 : slideWidth - slideWidth / 4);

				let deltaSlide = direction === -1 ? Math.ceil(computedDistance) : Math.round(computedDistance);
	...
	const animation = (start: number, target: number, duration = 1) => {
		const lerp = (start: number, end: number, time: number, r = easeOutCubic(time)) => start * (1 - r) + end * r;
